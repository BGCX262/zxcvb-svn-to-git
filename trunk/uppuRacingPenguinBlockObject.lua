...
function uppuRacingPenguinBlockObject:Destroy()
...
function uppuRacingPenguinBlockObject:CreateBody(pos, angle)
	-- 펭귄 스프라이트 로딩
	local object = Scene.latestScene["hsvIcePenguin"]:Clone()
	...
	objectDef.linearDamping = PENGUIN_BLOCK_LINEAR_DAMPING
	objectDef.angularDamping = PENGUIN_BLOCK_ANGULAR_DAMPING
					
	local objectShapeDef = b2PolygonDef()
	objectShapeDef:SetAsBox(TILE_SIZE_X/2, TILE_SIZE_Y/2)	-- 60x60
	objectShapeDef.density = PENGUIN_BLOCK_DENSITY
	objectShapeDef.restitution = PENGUIN_BLOCK_RESTITUTION
